...
## Warum Shield (und nicht Clerk)?

Clerk ist ein gehosteter Auth-Dienst für JS-Frameworks (Next.js etc.). Hier passt es
nicht:

- Das Backend ist **CodeIgniter (PHP)** — Clerk hat dafür kein First-Class-SDK.
- Die Nutzer sollen aus der mitgelieferten **`personen`-Tabelle** kommen, also lokal
  in der DB liegen — nicht in einem externen Identitätsanbieter.
- Rollen/Rechte (user / moderator / admin) sollen serverseitig in derselben DB
  erzwungen werden.

**Shield** ist das offizielle Auth-Paket für CodeIgniter 4: Sessions, Passwort-Hashing,
Gruppen und Permissions — alles lokal. Genau der richtige Fit.
