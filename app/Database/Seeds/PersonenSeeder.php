 ...
 *
 * Creates:
 *  - 1 admin      (admin / admin@team11.local / Admin123!)
 *  - 1 moderator  (moderator / moderator@team11.local / Moderator123!)
 *  - 9999 regular users from `personen`, all with password "password123"
 *    and email <username-transliterated>@example.com
 */
class PersonenSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->createAccount('admin', 'admin@team11.local', 'Admin123!', 'admin', $now);
        $this->createAccount('moderator', 'moderator@team11.local', 'Moderator123!', 'moderator', $now);

        // --- Bulk import of personen ------------------------------------
        // One shared hash: hashing 10k passwords individually would take minutes.
        $sharedHash = password_hash(self::DEFAULT_PASSWORD, PASSWORD_DEFAULT);
